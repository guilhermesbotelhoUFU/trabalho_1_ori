...
#Guilherme Botelho - 12311BSI217
#João Lucas Gonçalves - 12311BSI201
#Marcos Paulo - 12311BSI231
...
class SistemaRecuperacaoInformacao:
    ...
    def obter_indice_invertido(self):
        return dict(self.indice_invertido)
    ...
    def busca_frase(self, frase):
        ...
        for id_doc in docs_candidatos:
            ...
            for pos in listas_posicoes[0]:
                if self._verificar_sequencia(listas_posicoes, 1, pos + 1):
                    resultados.append(id_doc)
                    break 
        
        return resultados

    def _verificar_sequencia(self, listas_posicoes, indice_token, posicao_esperada):
        if indice_token >= len(listas_posicoes):
            return True
        
        posicoes_atuais = listas_posicoes[indice_token]
        if posicao_esperada in posicoes_atuais:
            return self._verificar_sequencia(listas_posicoes, indice_token + 1, posicao_esperada + 1)
        ...
        try:
            ...
            if escolha == '1':
                if indice_json < len(dados_json):
                    ...
                    sistema_ri.adicionar_documento(str(doc['id']), doc['texto'])
                    print(f"Documento ID {doc['id']} adicionado.")
                    indice_json += 1
                else:
                ...
                id_doc = input("Digite o ID do documento a remover: ")
